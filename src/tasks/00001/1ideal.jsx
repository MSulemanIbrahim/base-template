...
function Circle({ color, position, onClick }) {
  return (
    <div
      className={`absolute w-12 h-12 rounded-full cursor-pointer ${COLOR_CLASSES[color]}`}
...
export default function App() {
  ...
  useEffect(() => {
    if (gameState === "playing") {
      initGame();
      const gameInterval = setInterval(updateGame, 50); // Update every 50ms
  ...
  const initGame = () => {
    ...
    for (let i = 0; i < circleCount; i++) {
      ...
      do {
        ...
        isValid = !newCircles.some(
          (circle) =>
            Math.hypot(circle.position.x - position.x, circle.position.y - position.y) < 48
        );
      } while (!isValid);
        ...
        <>
          <div className="w-full flex justify-between p-4">
            ...
            <Alert className="w-64">
              ...
              <AlertDescription className={`font-bold ${COLOR_CLASSES[targetColor]}`}>
